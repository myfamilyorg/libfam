...
static void poly_compress11(uint8_t r[352 + 2], const poly *restrict a) {
	...
	const __m256i shufbidx = _mm256_set_epi8(
	    4, 3, 2, 1, 0, 0, -1, -1, -1, -1, 10, 9, 8, 7, 6, 5, -1, -1, -1, -1,
...
static void poly_decompress11(poly *restrict r, const uint8_t a[352 + 10]) {
	...
	const __m256i shufbidx =
	    _mm256_set_epi8(13, 12, 12, 11, 10, 9, 9, 8, 8, 7, 6, 5, 5, 4, 4, 3,
	...
	const __m256i shift = _mm256_set_epi16(4, 32, 1, 8, 32, 1, 4, 32, 4, 32,
					       1, 8, 32, 1, 4, 32);
...
#elif (KYBER_POLYVECCOMPRESSEDBYTES == (KYBER_K * 352))
	for (i = 0; i < KYBER_K; i++) poly_compress11(&r[352 * i], &a->vec[i]);
#endif
}
