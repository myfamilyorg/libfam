...
#include <emmintrin.h>
#include <immintrin.h>
#include <kyberavx2/KeccakP-1600-times4-SnP.h>
#include <kyberavx2/KeccakP-SIMD256-config.h>
#include <kyberavx2/KeccakP-align.h>
#include <kyberavx2/KeccakP-brg_endian.h>
...
#define LOAD4_64(a, b, c, d) \
	_mm256_set_epi64x((UINT64)(a), (UINT64)(b), (UINT64)(c), (UINT64)(d))
#define ROL64in256(d, a, o)                          \
	d = _mm256_or_si256(_mm256_slli_epi64(a, o), \
			    _mm256_srli_epi64(a, 64 - (o)))
...
#define STORE2_128(ah, al, v) \
	_mm256_storeu2_m128d((V128 *)&(ah), (V128 *)&(al), v)
...
#define PERM128(a, b, c) \
	(V256) _mm256_permute2f128_ps((__m256)(a), (__m256)(b), c)
#define SHUFFLE64(a, b, c) \
	(V256) _mm256_shuffle_pd((__m256d)(a), (__m256d)(b), c)
...
void KeccakP1600times4_AddLanesAll(void *states, const unsigned char *data,
	...
	const UINT64 *curData1 =
	    (const UINT64 *)(data + laneOffset * SnP_laneLengthInBytes);
	const UINT64 *curData2 =
	    (const UINT64 *)(data + laneOffset * 2 * SnP_laneLengthInBytes);
	const UINT64 *curData3 =
	    (const UINT64 *)(data + laneOffset * 3 * SnP_laneLengthInBytes);
	V256 lanes0, lanes1, lanes2, lanes3, lanesL01, lanesL23, lanesH01,
	    lanesH23;
...
#define Xor_In4(argIndex)                                  \
	...
	if (laneCount >= 16) {
		...
		if (laneCount >= 20) {
			Xor_In4(16);
			for (i = 20; i < laneCount; i++) Xor_In(i);
		} else {
			for (i = 16; i < laneCount; i++) Xor_In(i);
		}
	} else {
		for (i = 0; i < laneCount; i++) Xor_In(i);
	}
#undef Xor_In
#undef Xor_In4
}
...
#define OverWr4(argIndex)                                  \
	...
	if (laneCount >= 16) {
		...
		if (laneCount >= 20) {
			OverWr4(16);
			for (i = 20; i < laneCount; i++) OverWr(i);
		} else {
			for (i = 16; i < laneCount; i++) OverWr(i);
		}
	} else {
		for (i = 0; i < laneCount; i++) OverWr(i);
	}
#undef OverWr
#undef OverWr4
}
...
void KeccakP1600times4_ExtractLanesAll(const void *states, unsigned char *data,
	...
	UINT64 *curData1 =
	    (UINT64 *)(data + laneOffset * 1 * SnP_laneLengthInBytes);
	UINT64 *curData2 =
	    (UINT64 *)(data + laneOffset * 2 * SnP_laneLengthInBytes);
	UINT64 *curData3 =
	    (UINT64 *)(data + laneOffset * 3 * SnP_laneLengthInBytes);
...
#define Extr4(argIndex)                                             \
	...
	if (laneCount >= 16) {
		...
		if (laneCount >= 20) {
			Extr4(16);
			for (i = 20; i < laneCount; i++) Extr(i);
		} else {
			for (i = 16; i < laneCount; i++) Extr(i);
		}
	} else {
		for (i = 0; i < laneCount; i++) Extr(i);
	}
#undef Extr
#undef Extr4
}
...
void KeccakP1600times4_ExtractAndAddLanesAll(const void *states,
	...
	const UINT64 *curInput1 =
	    (UINT64 *)(input + laneOffset * 1 * SnP_laneLengthInBytes);
	const UINT64 *curInput2 =
	    (UINT64 *)(input + laneOffset * 2 * SnP_laneLengthInBytes);
	const UINT64 *curInput3 =
	    (UINT64 *)(input + laneOffset * 3 * SnP_laneLengthInBytes);
	UINT64 *curOutput0 = (UINT64 *)output;
	UINT64 *curOutput1 =
	    (UINT64 *)(output + laneOffset * 1 * SnP_laneLengthInBytes);
	UINT64 *curOutput2 =
	    (UINT64 *)(output + laneOffset * 2 * SnP_laneLengthInBytes);
	UINT64 *curOutput3 =
	    (UINT64 *)(output + laneOffset * 3 * SnP_laneLengthInBytes);
...
#define ExtrXor(argIndex)                                             \
	curOutput0[argIndex] =                                        \
	    curInput0[argIndex] ^ stateAsLanes64[4 * (argIndex)],     \
	curOutput1[argIndex] =                                        \
...
#define ExtrXor4(argIndex)                                                    \
	...
	if (laneCount >= 16) {
		...
		if (laneCount >= 20) {
			ExtrXor4(16);
			for (i = 20; i < laneCount; i++) ExtrXor(i);
		} else {
			for (i = 16; i < laneCount; i++) ExtrXor(i);
		}
	} else {
		for (i = 0; i < laneCount; i++) ExtrXor(i);
	}
#undef ExtrXor
#undef ExtrXor4
}
...
#endif
#include <kyberavx2/KeccakP-1600-unrolling.macros>
...
#undef Xor_In4
		...
		const UINT64 *curData0 = (const UINT64 *)data;
		const UINT64 *curData1 =
		    (const UINT64 *)(data + laneOffsetParallel * 1 *
						SnP_laneLengthInBytes);
		const UINT64 *curData2 =
		    (const UINT64 *)(data + laneOffsetParallel * 2 *
						SnP_laneLengthInBytes);
		const UINT64 *curData3 =
		    (const UINT64 *)(data + laneOffsetParallel * 3 *
						SnP_laneLengthInBytes);
		V256 *statesAsLanes = (V256 *)states;
		declareABCDE
...
#undef XOR_In
		...
		copyToState(statesAsLanes,
			    A) return (const unsigned char *)curData0 -
		    dataStart;
#endif
	} else {
		//        unsigned int i;
		...
		while (dataByteLen >=
		       (laneOffsetParallel * 3 + laneCount) * 8) {
